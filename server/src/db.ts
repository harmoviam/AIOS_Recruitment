...
export async function initDb() {
  ...
  try {
    ...
    await migrateHarmoviaCandidateSync(client);
...
/** Durable, retryable candidate delivery to Harmovia CRM. */
async function migrateHarmoviaCandidateSync(client: pg.PoolClient) {
  await client.query(`
    CREATE TABLE IF NOT EXISTS harmovia_candidate_sync_outbox (
    ...
    CREATE INDEX IF NOT EXISTS ix_harmovia_sync_pending
      ON harmovia_candidate_sync_outbox(status, next_attempt_at, id);

    CREATE OR REPLACE FUNCTION ${DB_SCHEMA}.enqueue_harmovia_candidate_sync()
    RETURNS TRIGGER AS $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM ${DB_SCHEMA}.tenants t
        WHERE t.id = NEW.tenant_id AND LOWER(t.slug) = 'harmovia'
      ) THEN
        RETURN NEW;
      END IF;
      INSERT INTO ${DB_SCHEMA}.harmovia_candidate_sync_outbox (
    ...
    DROP TRIGGER IF EXISTS trg_enqueue_harmovia_candidate_sync ON candidates;
    CREATE TRIGGER trg_enqueue_harmovia_candidate_sync
      ...
      EXECUTE FUNCTION ${DB_SCHEMA}.enqueue_harmovia_candidate_sync();

    CREATE OR REPLACE FUNCTION ${DB_SCHEMA}.enqueue_harmovia_job_candidates()
    RETURNS TRIGGER AS $$
    BEGIN
      INSERT INTO ${DB_SCHEMA}.harmovia_candidate_sync_outbox (
        tenant_id, candidate_id, status, attempts, next_attempt_at,
      ...
      FROM ${DB_SCHEMA}.candidates c
      JOIN ${DB_SCHEMA}.tenants t ON t.id = c.tenant_id AND LOWER(t.slug) = 'harmovia'
      WHERE c.job_id = NEW.id AND c.tenant_id = NEW.tenant_id
      ON CONFLICT (candidate_id) DO UPDATE SET
        status = 'pending',
        attempts = 0,
        next_attempt_at = NOW(),
    ...
    DROP TRIGGER IF EXISTS trg_enqueue_harmovia_job_candidates ON jobs;
    CREATE TRIGGER trg_enqueue_harmovia_job_candidates
      AFTER UPDATE OF title ON jobs
      FOR EACH ROW
      WHEN (OLD.title IS DISTINCT FROM NEW.title)
      EXECUTE FUNCTION ${DB_SCHEMA}.enqueue_harmovia_job_candidates();
  `);
}
